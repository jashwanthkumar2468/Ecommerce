...
<div class="content-wrapper">
	<section class="content-header">
		<h1>Manage Carat Type</h1> 
		<ol class="breadcrumb">
			<li><a href="<?php echo base_url(); ?>"><i class="fa fa-dashboard"></i> Home</a></li>
			<li class="active">Manage Carat Type</li>
		</ol> 
	</section> 
	<section class="content">
		...
		<div class="box box-default">
			<div class="box-header with-border">
				...
				<h3 class="box-title">Add New Carat Type</h3>
			...
			<div class="box-body" id="AddBdy" style="display:none">
				<form id="CategoryAddForm" onsubmit="return false;" method="post" enctype='multipart/form-data'>
					<div class="row">
						...
						<div class="col-md-3">
							<div class="form-group"> 
								<label>Carat Type</label>
								<input type="text" class="form-control required" name="caratname" id="caratname" placeholder="Enter Stone Name"/> 
							</div>
						</div>
					...
					<input type="hidden" id="CaratId" name="CaratId" />  
					<input type="hidden" name="Add_Edit_Carat_Type" value="true"/>
					<div class="row">  
						<div class="col-md-3"> 
		...
		<div class="box box-default">
			<div class="box-header with-border"> 
				<h3 class="box-title">Carat List</h3>
...
function get_carattype(page)
{
	var skey = $.trim($('#skey').val());
	$.ajax({
		url:"",
		method : "POST",
		data:{'get':'get_carattype','page':page,'skey':skey},
		success:function(data){
			data = $.parseJSON(data);
			var html='';
			if(data.get_carattype!='')
			{ 
				var n = 1;
				$.each(data.get_carattype, function(i){ 
					var row = data.get_carattype[i];
					var status = '';
					var $n = n++; 
					if(row.Status == '1')  
					...
					html+=" <tr><td>"+$n+"</td><td>"+row.caratname+"</td><td><button class='btn btn-primary btn-xs' onclick='edit("+row.CaratId+")'><i class='fa fa-pencil'></i></button>&nbsp;&nbsp;&nbsp;<button class='btn btn-danger btn-xs' onclick='del("+row.CaratId+")'><i class='fa fa-trash'></i></button></td></tr>";
				});
			} 
			else
...
function edit(id)
{
	...
	if(id!='')
	{
		$.ajax({
			url:"",
			method : "POST", 
			data:{'get':'caratedit','eid':id},
			success:function(data){
				data = $.parseJSON(data); 
				var html='';  
				if(data.installation!='')
				{
					$('#CaratId').val(data.CaratId); 
					$('#caratname').val(data.caratname); 
...
function del(id)
{
	...
	if(id!='')
	{
		if(confirm('Do you really want to delete this Carat ?')) 
		{
			$.ajax({
				url:"", 
				method : "POST",
				data:{'Del':'Del_carat','iid':id},
				success:function(data){     
					window.location.reload();         
				} 
			}); 
...
{
	var invalid				= '';
	var caratname 			= $.trim($('#caratname').val());
	...
	{ 
		invalid = 1;
		$('#caratname').css('border-color','red');
	}
	else
		$('#caratname').css('border-color',''); 
